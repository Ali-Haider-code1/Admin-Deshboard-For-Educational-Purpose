...
$assignmentId = $_GET['assignmentId']; // Retrieve course ID from URL parameter
...
if ($result) {
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $lectureFilename = $row['assignment'];
    $lecturePath = 'assignments/' . $lectureFilename;
  }
}
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = "UPDATE assignment_details SET coursecode=?, coursename=?, teachername=? WHERE id=?";
  $stmt = mysqli_prepare($con, $sql);

  if ($stmt) {
    mysqli_stmt_bind_param($stmt, "sssi", $editedCourseCode, $editedCourseName, $editedTeacherName, $assignmentId);

    if (mysqli_stmt_execute($stmt)) {
      echo "Assignment details updated successfully.";
      // Redirect back to the course details page
      header("Location: assignment.php"); // Change to the actual URL
    } else {
              ...
              <form role="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
                <div class="row">
                  <div class="col-xl-12 col-lg-5 col-md-7 mx-auto">
                    <div class="mb-3">
                      <input type="hidden" name="courseId" value="<?php echo $assignmentId; ?>">
